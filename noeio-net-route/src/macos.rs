...
pub fn add_route(target: IpAddr, prefix: u8, ifindex: u32, metric: u32) -> io::Result<()> {
    ...
    let ifindex = u16::try_from(ifindex).map_err(|_| {
        io::Error::new(
            io::ErrorKind::InvalidInput,
    ...
    message.header.rtm_type = libc::RTM_ADD as u8;
    message.header.rtm_flags = libc::RTF_UP | libc::RTF_STATIC;
    message.header.rtm_addrs = libc::RTA_DST | libc::RTA_GATEWAY | libc::RTA_NETMASK;
